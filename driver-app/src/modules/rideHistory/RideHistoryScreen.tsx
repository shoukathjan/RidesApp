import { BookingStatusLabel } from '@useme/shared';
...
export default function RideHistoryScreen() {
  ...
  useEffect(() => {
    // Reuses the driver's completed/claimed rides via earnings-style listing.
    api
      .get('/driver/earnings')
      .then(() => api.get<Ride[]>('/bookings/history').catch(() => ({ data: [] })))
      .then((r: { data: Ride[] }) => setRides(r.data))
      .catch(() => setRides([]));
  }, []);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.title}>₹{item.fareEstimate}</Text>
            <Text style={styles.sub}>{BookingStatusLabel[item.status]}</Text>
            <Text style={styles.sub}>{new Date(item.scheduledAt).toLocaleString()}</Text>
          </View>
        )}
      />
    </View>
